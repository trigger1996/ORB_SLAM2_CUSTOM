...
int __lidar_driver::init() {
	...
	if (!opt_com_path) {
		...
		opt_com_path = "\\\\.\\com3";
#else
        opt_com_path = (char *)"/dev/ttyUSB0";	// /dev/ttyUSB0
#endif
	}
...
void __lidar_driver::fix_Data_withAHRS(double pitch, double roll, double yaw, double is_fix_yaw) {
    // 输入的角度应为弧度制
    int i;

    // 仅对小角度有效
    const static double max_angle = 30.0f;
    if (fabs(pitch) >= max_angle * CV_PI / 180.0f || fabs(roll) >= max_angle * CV_PI / 180.0f)
        return;

    for (i = 0; i < laserData.size(); i++) {
        double theta = laserData[i].angle;
        double rho   = laserData[i].dst;
        double x, y;
        ...
        laserData[i].dst = sqrt(x * x + y * y);

        if (is_fix_yaw) {
            laserData[i].angle += -yaw * 180.0f / CV_PI;
        }
    }
